    ...
    public function user_level($user_id,$level=0,$step_num=0,$add_score=0,$jb_score=0){
        ...
        if($level==0){
            ...
            if($tz_step>$step_num&&$step_num>= $wn_step){
                $wn_count_days = Db::name('user_run')->where("add_time >=$range_days and user_id= $user_id and step_num>=$wn_step" )->count('id');
                //判断是否达到了蜗牛达人的要求
                ...
                $tz_count_days = Db::name('user_run')->where("add_time >=$range_days and user_id= $user_id and step_num>=$tz_step" )->count('id');
                //判断是否达到了兔子达人的要求
                if($tz_count_days>=$tz_upgrade_days){
                    $res = Db::name('user')->where('id',$user_id)->setfield('level',$level+2);//达到兔子达人
                }
            }elseif($step_num>=$lb_step){
                $lb_count_days = Db::name('user_run')->where("add_time >=$range_days and user_id= $user_id and step_num>=$lb_step" )->count('id');
                //判断是否达到了猎豹达人的要求
            ...
            if($lb_step>$step_num&&$step_num>=$tz_step){
               $tz_count_days = Db::name('user_run')->where("add_time >=$range_days and user_id= $user_id and step_num>=$tz_step" )->count('id');
                ...
                $step_count_days = Db::name('user_run')->where("add_time >=$range_days and user_id= $user_id and step_num<$wn_step" )->count('id');
            ...
            if($step_num>=$lb_step){
                ...
                $step_count_days = Db::name('user_run')->where("add_time >=$range_days and user_id= $user_id and step_num<$tz_step" )->count('id');
            ...
            if($step_num>=$tz_step&&$step_num<$lb_step){
                $step_count_days = Db::name('user_run')->where("add_time >=$range_days and user_id= $user_id and step_num<$lb_step" )->count('id');
